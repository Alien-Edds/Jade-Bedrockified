import { Block, Entity, EntityHealthComponent, Player, RawText, system, world } from "@minecraft/server";
...
function getBlockMessage(block: Block): (string | RawText)[] {
    ...
    for (let i = 0; i < Object.keys(states).length; i++) {
        ...
        if (i === 3) text.push(` & ${Object.keys(states).length - 3} more rows...`)
...
function getHealthEmojis(current: number, max: number): string {
    ...
    if (max > 100 && currentHealth > 100) return `§c${currentHealth}§f/§c${max}${Emojis.HeartFull}`
    if (max > 100 && currentHealth <= 100) max = 100
...
world.afterEvents.entityHitEntity.subscribe((data) => {
    ...
    (data.damagingEntity as Player).onScreenDisplay.setActionBar(getEntityMessage(data.hitEntity))
})